...
export function createVendorAdminRoutes(): Router {
  ...
  /* --- batch / cascade (stubs until other services exist) --- */
  const stubBatch =
    (resource: Parameters<VendorCascadeService['acknowledgeBatch']>[1]) =>
    async (req: Request, res: Response) => {
      try {
        const body = await cascade.acknowledgeBatch(req.params.vendorId, resource);
        res.json(body);
      } catch (e: any) {
        res.status(500).json({ message: e.message });
      }
  ...
  r.delete('/serviceNotifications/vendors/:vendorId', async (req: Request, res: Response) => {
    try {
      const body = await cascade.deleteServiceNotificationsForVendor(req.params.vendorId);
      res.json(body);
    } catch (e: any) {
      res.status(500).json({ message: e.message });
    }
  ...
  /* paged orders for vendor — order service not integrated */
  r.get('/orders/vendors/:vendorId', async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        limit,
        offset,
        note: 'Order service not integrated; wire when order-management-service exists.',
      });
    } catch (e: any) {
      res.status(500).json({ message: e.message });
    }
